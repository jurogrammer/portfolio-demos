    ...
    <div>
      <section className="py-24 bg-[#00194a] text-white">
        <div className="px-8 lg:px-16">
          <h1 className="font-serif text-[48px] lg:text-[68px] font-medium mb-4">{isEn ? 'News & Blog' : '뉴스 & 블로그'}</h1>
          <p className="text-white/70 text-[18px]">{isEn ? 'Latest news and tech insights' : '최신 소식과 기술 인사이트'}</p>
        </div>
      </section>
          ...
          {posts.length === 0 ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {posts.map(post => (
                  <Link key={post.id} href={`/${locale}/news/${post.slug}`} className="block border border-[#d4d4d4] overflow-hidden hover:opacity-75 transition-opacity">
                    <div className={`h-40 ${post.category === 'blog' ? 'bg-[#111111]' : 'bg-[#00194a]'}`} />
                    <div className="p-4">
                      ...
                      <h3 className="font-bold text-[#111111] line-clamp-2">
                ...
                <div className="flex justify-center gap-2">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                    <Link
                      ...
                      className={`px-4 py-2 border rounded ${p === page ? 'bg-[#00194a] text-white border-[#00194a]' : 'hover:bg-[#f5f6fb]'}`}
                    >
                      {p}
                    </Link>
                  ))}
